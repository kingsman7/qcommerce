            ...
            <q-form autocorrect="off" autocomplete="off" ref="formContent" class="box"
               ...
               <div class="row q-col-gutter-x-md" v-if="success">
                  ...
                  <div class="col-12 col-md-8" v-if="locale.success">
                     ...
                     <!--Sumario-->
                     <div class="input-title">{{`${$tr('ui.form.summary')} (${locale.language})*`}}</div>
                     <q-field v-model="locale.formTemplate.summary" borderless
                              :rules="[val => !!val || $tr('ui.message.fieldRequired')]">
                        <q-editor v-model="locale.formTemplate.summary" class="full-width"
                     ...
                     <!--Meta Description-->
                     <q-input v-model="locale.formTemplate.metaDescription" outlined dense type="textarea"
                  ...
                  <div class="col-12 col-md-4" v-if="locale.success">
                     ...
                     <div v-if="canManageRecordMaster" class="q-mb-md">
                        ...
                        <q-select
                                ...
                                :options="[
                                  {label: this.$tr('ui.label.yes'), id: 1, value:1},
                                  {label: this.$tr('ui.label.no'), id: 0, value:0},
                                ]"
                                map-options
                                emit-value
                                option-label="label"
                     ...
                     <q-select
                             ...
                             class="q-mb-md"
                             :options="optionsTemplate.optProd"
                             map-options
                             emit-value
                             use-input
                             option-label="label"
                             :filter="(val, update)=>update(()=>{optionsTemplate.optProd = $helper.filterOptions(val,optionsTemplate.products,form.storeTitle)})"
                             placeholder=""
                     />
                     <!--Status-->
                     ...
                     <q-select
                             ...
                             option-label="label"
                             class="q-mb-md"
                             :options="optionsTemplate.optCate"
                             v-model="locale.formTemplate.categoryId"
                             :filter="(val, update)=>update(()=>{optionsTemplate.optCate = $helper.filterOptions(val,optionsTemplate.optionsTemplate.category,form.storeTitle)})"
                             use-input

                             placeholder=""
                     />
                     <crud v-model="locale.formTemplate.categoryId"
                           :crud-data="import('@imagina/qcommerce/_crud/productCategories')"
                           type="button"
                           @created="getCategories"
                           :crud-props="{label:`${$tr('ui.form.category')}*`}"
                     />
                     <!--Categories-->
   ...
   export default {
      ...
      methods: {
         ...
         getCategories() {
            return new Promise((resolve, reject) => {
               ...
               this.$crud.index(configName, params).then(response => {
                  this.optionsTemplate.category= this.$array.select(response.data, {id: 'id', label: 'title'});
                  this.optionsTemplate.categories = this.$array.tree(response.data, {id: 'id', label: 'title'});
                  this.optionsTemplate.optCate = this.$clone(this.optionsTemplate.category)
         ...
         orderDataItemToLocale(data) {
            ...
            if (data.parent) this.optionsTemplate.products = this.$array.select(data.parent,{label:'name',id:'id'})
            //Order categories
            ...
            orderData.categoryId=parseInt(orderData.categoryId)
            orderData.status=parseInt(orderData.status)
